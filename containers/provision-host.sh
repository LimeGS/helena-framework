...
image="${HELENA_WORKER_IMAGE:-helena-worker:latest}"
...
scp -q "$here/containers/systemd/helena-control-tunnel.service" \
       "$here/containers/systemd/helena-worker.service" "$target:/tmp/"
ssh "$target" 'sudo install -m 0644 /tmp/helena-control-tunnel.service /etc/systemd/system/
  sudo install -m 0644 /tmp/helena-worker.service /etc/systemd/system/
  rm -f /tmp/helena-*.service
  sudo systemctl daemon-reload
  sudo systemctl enable --now helena-control-tunnel
  sudo systemctl reset-failed helena-worker 2>/dev/null || true
  sudo systemctl enable --now helena-worker'
...
ssh "$target" 'printf "    tunnel: %s\n    worker: %s\n" \
    ...
    "$(systemctl is-active helena-worker)"
  printf "    container: %s\n" "$(sudo docker ps --format "{{.Names}} {{.Status}}" \
    | grep helena-worker || echo "not running")"'
