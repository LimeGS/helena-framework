...
def survey_inputs(dataset: Path, profile: dict[str, Any],
    ...
    def present(path: str) -> bool:
        # Empty first: an unset layout value (spiral-fitter-v1@0.4.1's own
        # unverified_patches_dir, empty by default -- see adapter.py's
        # DEFAULT_UNVERIFIED_PATCHES_DIR) resolves "{unverified_patches_dir}"
        # to "", and `dataset / ""` is `dataset` itself in pathlib -- which
        # always exists. Read literally that says the input is present; what
        # it actually means is the input was never named. A path may still be
        # a glob; a directory is a present .ome.zarr.
        if not path:
            return False
    ...
    absent_optional = [
        # The template itself, not "", when an unset layout value resolved to
        # nothing -- "" in a receipt reads as a typo, not as "not asked for".
        {"path": resolve(entry["path"]) or entry["path"],
         "costs": entry.get("absence_costs")}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--unverified-patches-dir",
                        help="one directory directly under the dataset root "
                             "holding grown, unverified track-graph patches "
                             "(e.g. the grow-track-patches lane's own output). "
                             "Unset means not asked for: no path override is "
