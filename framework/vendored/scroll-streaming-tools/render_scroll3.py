"""Chunk-gather surface renderer for the PHerc 0332 (Scroll 3) 2.399 um
rescan. Takes a segment PPM in the legacy frame, bridges through the
published transform.json (landmark-verified), and streams exactly the
128^3 chunks each output stripe needs. Prints the byte budget per stripe
before downloading; per-stripe checkpoint in done.json makes any crash
resumable with the same command.

Usage: python render_scroll3.py --ppm seg.ppm --out out/seg \
         [--layers 62 --spacing 1.0 --stripe 640 --conc 64 --max-gb 14]
(--spacing defaults to the matrix-derived legacy step; pass 1.0 for
native-resolution layer steps, e.g. 62-layer canonical-style stacks)
"""
...
NOMINAL_SPACING = 7.91 / 2.399   # voxel-size ratio, for reference only
CH = 128
VOLSHAPE = (33592, 15761, 15761)          # z,y,x from the .zarray metadata
