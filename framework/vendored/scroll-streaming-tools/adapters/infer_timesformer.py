# Patched copy of ScrollPrize/villa ink-detection/inference_timesformer.py
# (fetched 2026-07-01). Model, tiling math, and prediction assembly are
# UNCHANGED. Patches, each marked "# PATCH":
#   1. respect pre-set CUDA_VISIBLE_DEVICES (original clobbered it -> both
#      processes would land on GPU 0)
#   2. lazy tile cropping in the Dataset (original materialized every 64x64x26
#      tile up-front: ~7 GB RAM per big segment; box has 24 GB for 2 procs)
#   3. wandb removed (inference needs no experiment logging)
...
def read_image_mask(fragment_id,start_idx=18,end_idx=38,rotation=0):
    ...
    if any(id_ in fragment_id for id_ in ['20230701020044','verso','20230901184804','20230901234823','20230531193658','20231007101615','20231005123333','20231011144857','20230522215721', '20230919113918', '20230625171244','20231022170900','20231012173610','20231016151000']):
        ...
        images=images[:,:,::-1]
    elif args.reverse:  # PATCH 5: original parsed --reverse but never applied it
        print("Reverse Segment (via --reverse flag)")
        images=images[:,:,::-1]
    fragment_mask=None
    ...
    for y1 in y1_list:
        for x1 in x1_list:
            ...
            if not np.any(fragment_mask[y1:y2, x1:x2]==0):
                xyxys.append([x1, y1, x2, y2])  # PATCH 2: coords only, no crop copies
    test_dataset = CustomDatasetTest(image,np.stack(xyxys), CFG,transform=A.Compose([
...
class CustomDatasetTest(Dataset):
    def __init__(self, image, xyxys, cfg, transform=None):  # PATCH 2
        self.image = image          # full (H, W, C) stack, shared
        self.xyxys=xyxys
        self.cfg = cfg
    ...
    def __getitem__(self, idx):
        xy=self.xyxys[idx]
        x1, y1, x2, y2 = xy         # PATCH 2: crop lazily
